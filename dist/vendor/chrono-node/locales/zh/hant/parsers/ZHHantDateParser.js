...
var MONTH_GROUP = 2;
var DAY_GROUP = 3;
var ZHHantDateParser = exports["default"] = /*#__PURE__*/function (_AbstractParserWithWo) {
  function ZHHantDateParser() {
    _classCallCheck(this, ZHHantDateParser);
    return _callSuper(this, ZHHantDateParser, arguments);
  }
  _inherits(ZHHantDateParser, _AbstractParserWithWo);
  return _createClass(ZHHantDateParser, [{
    key: "innerPattern",
    value: function innerPattern() {
      return new RegExp("(" + "\\d{2,4}|" + "[" + Object.keys(_constants.NUMBER).join("") + "]{4}|" + "[" + Object.keys(_constants.NUMBER).join("") + "]{2}" + ")?" + "(?:\\s*)" + "(?:年)?" + "(?:[\\s|,|，]*)" + "(" + "\\d{1,2}|" + "[" + Object.keys(_constants.NUMBER).join("") + "]{1,2}" + ")" + "(?:\\s*)" + "(?:月)" + "(?:\\s*)" + "(" + "\\d{1,2}|" + "[" + Object.keys(_constants.NUMBER).join("") + "]{1,2}" + ")?" + "(?:\\s*)" + "(?:日|號)?");
    }
  }, {
    key: "innerExtract",
