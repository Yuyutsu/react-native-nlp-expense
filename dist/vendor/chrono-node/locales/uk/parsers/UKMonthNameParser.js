...
var YEAR_GROUP = 3;
var UkMonthNameParser = exports["default"] = /*#__PURE__*/function (_AbstractParserWithLe) {
  function UkMonthNameParser() {
    _classCallCheck(this, UkMonthNameParser);
    return _callSuper(this, UkMonthNameParser, arguments);
  }
  _inherits(UkMonthNameParser, _AbstractParserWithLe);
  return _createClass(UkMonthNameParser, [{
    key: "innerPatternString",
    value: function innerPatternString(context) {
      return "((?:\u0432|\u0443)\\s*)?" + "(".concat((0, _pattern.matchAnyPattern)(_constants.MONTH_DICTIONARY), ")") + "\\s*" + "(?:" + "[,-]?\\s*(".concat(_constants.YEAR_PATTERN, ")?") + ")?" + "(?=[^\\s\\w]|\\s+[^0-9]|\\s+$|$)";
    }
  }, {
    key: "innerExtract",
    value: function innerExtract(context, match) {
      ...
      if (match[YEAR_GROUP]) {
        var year = (0, _constants.parseYearPattern)(match[YEAR_GROUP]);
        result.start.assign("year", year);
      } else {
        var _year = (0, _years.findYearClosestToRef)(context.reference.instant, 1, month);
        result.start.imply("year", _year);
      }
      return result;
    }
  }]);
}(_AbstractParserWithWordBoundaryChecking.AbstractParserWithLeftBoundaryChecking);